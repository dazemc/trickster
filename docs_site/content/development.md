...
Work happens on one branch per phase (`bar/phase-1`, `bar/phase-2`, …), each step committed separately as it lands proven and is removed from `.llm/todo.md`, with the phase merged back into `main` through a pull request when done. Commits follow `scope: summary` in the imperative.
