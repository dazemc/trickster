...
- [ ] **1.4 battery bolt in-cell** (S). `lib/src/bar/battery.dart`. Draw the
...
- [ ] **1.5 battery tap action** (S). `lib/src/bar/battery.dart`. Migrate the
...
- [ ] **1.6 CPU sample series** (S). `lib/src/services/cpu.dart`. Keep a
      bounded series of recent samples (cap + test the cap). Done when:
      unit test asserts bound and ordering.
- [ ] **1.7 CPU sparkline pill** (S). `lib/src/bar/cpu.dart`. Render the
...
- [ ] **1.8 GPU sampler service** (S). `lib/src/services/gpu.dart` (new).
...
- [ ] **1.9 GPU cards in strip** (S). `lib/src/bar/bar.dart` + new
...
- [ ] **1.10 workspace rail widget** (M). `lib/src/bar/workspaces.dart`.
...
- [ ] **1.11 rail occupied/urgent states** (S). Same file. Derive occupied
      from backend flags; urgent styling from telemetry colors. Done when:
      all three states render distinctly, tested.
- [ ] **1.12 rail click targets** (S). Same file. Migrate pips onto
...
- [ ] **1.13 sway focus verb** (S). `lib/src/services/workspaces.dart`. Send
...
- [ ] **1.14 hyprland focus verb** (S). Same file. `dispatch workspace` over
      the existing event/command sockets. Done when: same bar as 1.13.
- [ ] **1.15 niri focus verb** (S). Same file. `focus-workspace` over the
      existing stream. Done when: same bar as 1.13.

## Phase 2 — parity (absent modules)

- [ ] **2.1 StatusNotifier core** (M). New
...
- [ ] **2.5 MPRIS service** (M). New `lib/src/services/mpris.{dart,test}`.
...
- [ ] **2.6 media pill + controls** (S). New `lib/src/bar/media.dart`.
...
- [ ] **3.1 arb pipeline + EN** (M). `lib/l10n/*.arb`, `flutter gen-l10n`
...
- [ ] **3.2 zh strings** (S). Port Denial's bar strings for clock, battery,
      tray status, workspace semantics. Done when: zh widget tests.
- [ ] **3.3 blur capability probe** (S). Probe `ext-background-effect` once
      at startup, expose as a provider. Done when: tested both ways via
      override.
- [ ] **3.4 pill backdrop integration** (S). `lib/src/bar/pill.dart`. Blur
      behind pills when capable, translucent fill otherwise. Done when:
      exercised on compositors with and without the protocol.
- [ ] **3.5 font bundle** (S). Measure painted glyphs; bundle the JetBrainsMono
...
- [ ] **3.6 output enumeration + selection** (S). Bootstrap + layer-shell
      platform code. Enumerate monitors, honor `outputs.conf` connector
      selection. Done when: selection logic unit-tested.
- [ ] **3.7 per-output surface lifecycle** (S). Create one layer surface per
...
- [ ] **3.8 control socket transport** (S). Socket transport behind the
...
- [ ] **3.9 tricksterctl status + version** (S). `bin/tricksterctl.dart`.
      Short-lived client only, never a second UI runtime. Done when: both
      commands round-trip against a live bar.
- [ ] **3.10 tricksterctl reload** (S). Same binary. Re-read configs through
      the same path as the file watcher. Done when: reload applies a config
      edit end to end.
- [ ] **3.11 workspace options schema** (S). `lib/src/config/settings.dart`.
