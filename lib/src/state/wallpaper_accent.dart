...
class WallpaperAccentController extends ChangeNotifier {
  ...
  Color? accentFor(String output) => _accents[output] ?? _color;

  /// [output]'s wallpaper candidates, strongest first; falls back to the
  /// first sampled output's list when the cache does not name [output].
  List<Color> candidatesFor(String output) {
    final own = _candidates[output];
